<div class="h-full flex flex-col justify-between p-4">
        ...
        <ul class="space-y-1">
            @forelse ($menus as $menu)
             @php $isActive = request()->is(ltrim($menu->url, '/').'*'); @endphp
                <li>
                    <a href="{{ url($menu->url) }}" class="flex items-center px-3 py-2 rounded-lg text-sm transition-colors duration-200 {{ $isActive ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700' }}">
    ...
    <div class="border-t pt-4">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700">
