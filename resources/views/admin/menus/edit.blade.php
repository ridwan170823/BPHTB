...
<div class="p-6 max-w-xl mx-auto bg-white rounded-lg shadow-md">
    ...
    <form method="POST" action="{{ route('admin.menus.update', $menu) }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $menu->title) }}" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm" required>
            @error('title') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">URL</label>
            <input type="text" name="url" value="{{ old('url', $menu->url) }}" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm" required>
            @error('url') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Icon</label>
            <input type="text" name="icon" value="{{ old('icon', $menu->icon) }}" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
        ...
        <div>
            ...
            <select name="role" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                @foreach (App\Models\User::ROLES as $key => $label)
                    <option value="{{ $key }}" {{ old('role', $menu->role) == $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="number" name="order" value="{{ old('order', $menu->order) }}" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
